...
monthsBetween dA dB =
    ...
    case(date1.month , date2.month) of
        (Just m1 , Just m2) -> Just (abs((date1.year - date2.year)*12 + monthToInt m1 - monthToInt m2))
        (Just _, Nothing) -> Nothing
        (Nothing, Just _) -> Nothing
        (Nothing, Nothing) -> Just (abs(date1.year - date2.year)* 12) 
...
compare (Date d1) (Date d2) =
    let
        yearComp= Basics.compare d1.year d2.year
    in
        if yearComp == EQ then
...
view : Date -> Html msg
view (Date d) =
    div [] [text (String.fromInt d.year)
            , case d.month of
            Just m -> Html.span [] [ text (" " ++ monthToString m) ]
            Nothing ->text ""]
