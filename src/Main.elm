...
init : () -> ( Model, Cmd Msg )
init _ = update GetRepos initModel
    -- ( initModel
    -- , fetchRepos
    -- -- [(GetRepos, fetchRepos)]
    -- )
...
update msg model =
    case msg of
        ...
        GotRepos (Ok repoList) ->
            ( {model | repos = repoList}, Cmd.none )
...
fetchRepos: Cmd Msg
fetchRepos = Http.get{url = "https://api.github.com/Toderean/repos", expect = Http.expectJson GotRepos (De.list Repo.decodeRepo)}
